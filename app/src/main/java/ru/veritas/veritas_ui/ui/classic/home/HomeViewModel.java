...
public class HomeViewModel extends ViewModel {
    ...
    public HomeViewModel(CommandFactory commandFactory,
    ...
    public void loadShortcuts() {
        executor.execute(() -> {
            try {
                var shortcuts = getShortcutsUseCase.invoke();
    ...
    public void loadFavorites() {
        executor.execute(() -> {
            var raw = getFavoritesUseCase.invoke();
    ...
    public void addShortcutToDesktop(AppShortcut shortcut, int page, int row, int col) {
        execute(commandFactory.createAddShortcutToDesktopCommand(shortcut, page, row, col));
    }

    public void moveShortcut(int fromPage, int fromRow, int fromCol,
                             int toPage, int toRow, int toCol) {
        execute(commandFactory.createMoveShortcutCommand(fromPage, fromRow, fromCol, toPage, toRow, toCol));
    }

    public void swapShortcutWithFavoriteAndHome(int desktopPage, int desktopRow, int desktopCol,
                                                int favPage, int favPos) {
        execute(commandFactory.createSwapShortcutWithFavoriteCommand(desktopPage, desktopRow, desktopCol, favPage, favPos));
    }

    // --- Действия с избранным ---
    public void addToFavorites(AppShortcut shortcut, int page, int pos) {
        execute(commandFactory.createAddToFavoritesCommand(shortcut, page, pos));
    }

    public void removeFromFavorites(int page, int pos) {
        execute(commandFactory.createRemoveFromFavoritesCommand(page, pos));
    }

    public void swapFavorites(int srcPage, int srcPos, int dstPage, int dstPos) {
        execute(commandFactory.createSwapFavoritesCommand(srcPage, srcPos, dstPage, dstPos));
    }
